...
(defun jenkins-sort-by-id (a b)
  "Sort two table entries A and B by their status."
  (let ((status-a (format "%s" (eval (get-text-property 0 'face (aref (cadr a) 0)))))
        (status-b (format "%s" (eval (get-text-property 0 'face (aref (cadr b) 0))))))
    (string< status-a status-b)))

(defun jenkins-list-format ()
  "List of columns for main jenkins jobs screen."
  (apply 'vector
  `(("#" ,jenkins-colwidth-id jenkins-sort-by-id :pad-right 2 :right-align t :col-source jenkins--render-indicator)
	("Name" ,jenkins-colwidth-name t :col-source jenkins--render-name)
	("Last success" ,jenkins-colwidth-last-status nil :col-source :last-success)
	("Last failed" ,jenkins-colwidth-last-status nil :col-source :last-failed))
 ))
...
(defun jenkins-views-url ()
  "Jenkins url for getting a list of views."
  (format "%sapi/json?depth=2&tree=name,views[name]" (get-jenkins-url)))
...
(defun jenkins--retrieve-list-of-views ()
  "Retrieve a list of views in jenkins."
  (with-current-buffer (url-retrieve-synchronously (jenkins-views-url))
    (goto-char (point-min))
    (re-search-forward "^$")
    (delete-region (point) (point-min))
    (let ((views (cdr (assoc 'views (json-read-from-string (buffer-string))))))
      (--map (cons (cdr (assoc 'name it)) (url-hexify-string (cdr (assoc 'name it)))) views))))
...
(defun jenkins-get-jobs-list (view)
...
;;;###autoload
(defun jenkins (&optional pfx)
  "Initialize jenkins buffer.
When called with a PFX argument, prompt the user for a view to load"
  (interactive "P")
  (jenkins--setup-variables)
  (let ((viewname
         (when pfx
           (let* ((views (jenkins--retrieve-list-of-views))
                  (view (completing-read "Jenkins View: " views)))
             (cdr (assoc view views))))))
    (switch-to-buffer-other-window (format "%s:%s" jenkins-buffer-name view))
    (with-current-buffer jenkins-buffer-name
      (kill-all-local-variables)
      (setq-local jenkins-viewname viewname)
      (message "%s" jenkins-viewname)
      (read-only-mode 0)
      (erase-buffer)
      (read-only-mode 1)
      (jenkins-mode))))
