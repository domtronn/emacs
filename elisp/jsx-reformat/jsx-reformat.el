...
(defun jsx--reformat-tag ()
  (let ((start-point (point))
    ...
    (when  (not match-line)
      (unless (looking-back "^\s+") (smart-newline)))
...
(defun jsx--reformat-element-attributes ()
  (let ((count (jsx--count-attributes))
        (limit (web-mode-tag-end-position)))
    (when (> count 1)
      (while (and (web-mode-attribute-next) (< (point) limit))
        (unless (looking-back "^\s+") (smart-newline))))))
...
(defun jsx-reformat ()
  ...
  (save-excursion
    (let*
        ((start (if (region-active-p) (region-beginning) (1+ (search-backward "("))))
         (end (if (region-active-p) (region-end)  (- (search-forward ")") 1)))
     ...
     (with-temp-buffer
       ...
       (setq result (format "\n%s\n" (s-chomp (s-trim (buffer-string))))))
     (delete-region start end)
     (goto-char start)
     (insert result)
     (indent-region start (search-forward ")")))))
